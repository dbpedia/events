...
public class DigestItem {
    ...
    public DigestItem(DBpediaLiveDigest digest, DigestTemplate digestTemplate, String res, String u, DateTime start, DateTime end) {
        this.digest = digest;
    ...
    public DigestItem(DBpediaLiveDigest digest, DigestTemplate digestTemplate, Map<String, RDFNode> bindings, DateTime start, DateTime end) {
        this(digest, digestTemplate, bindings.get("res").asResource().getURI(), bindings.get("u").asResource().getURI(), start, end);
    ...
    public String getId() {
        return this.digest.getDigestId(this.start, this.end) +
    ...
    public Resource getAsResource(Model model) {
        ...
        event.addProperty(ProvOntology.generatedAtTime, model.createTypedLiteral(this.digest.getEnd(this.end), XSDDatatype.XSDdateTime));
        ...
        digest.setQueryExecutionDatetime(qe, digest.getEnd(this.end));
        ResultSet rs = qe.execSelect();
        if (!rs.hasNext()) {
            Logger.debug("No modified date.");
        } else while (rs.hasNext()) {
            QuerySolution r = rs.nextSolution();
            String days = r.get("days").asLiteral().getString();
            Logger.debug("Unmodified days = " + days);
            if (!days.startsWith("-")) {
